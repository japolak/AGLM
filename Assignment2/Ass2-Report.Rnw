...
We compare the two models with the anova function and a chi-squared-test. The statistics $G$ takes value $6.7045$, has $\chi^2$ distribution and p-value $0.035$. Thus at significance level $\alpha = 0.05$ we reject $H_0 : \beta_{PrevArt} = \beta_{PrevCin} = 0$ and conclude that Model 2 including PrevArt and PrevCin has a better fit than Model 1. \\
...
\textbf{Income} $\beta_{Income}$ = \Sexpr{mod.fin$coefficients["Income"]}: An increase in income by 1000, increases the odds multiplicatively to buy the magazine by the factor $\exp(\beta_{Income} * 1000)= \Sexpr{odds_inc}$.\\

\textbf{Is.Female} $\beta_{Is.Female}$ = \Sexpr{mod.fin$coefficients["Is.Female"]}: The customer being female increases the odds multiplicatively to buy the magazine by the factor $\exp(\beta_{Is.Female})=\Sexpr{odds_fem}$ \\

\textbf{Is.Retired} $\beta_{Is.Retired}$ = \Sexpr{mod.fin$coefficients["Is.Retired"]}: The customer being retired decreases the odds multiplicatively to buy the magazine by the factor $\exp(\beta_{Is.Retired})=\Sexpr{odds_ret}$\\

\textbf{Residence.Length} $\beta_{Residence.Length}$ = \Sexpr{mod.fin$coefficients["Residence.Length"]}: An increase of the length of residence in the current city by 1 year, increases the odds multiplicatively to buy the magazine by the factor $\exp(\beta_{Residence.Length})=\Sexpr{odds_res}$\\

\textbf{Dual.Income} $\beta_{Dual.Income}$ = \Sexpr{mod.fin$coefficients["Dual.Income"]}: If the household considered has a dual income, the odds to buy the magazine are increased multiplicatively by the factor $\exp(\beta_{Dual.Income})=\Sexpr{odds_dual}$ \\

\textbf{Minors} $\beta_{Minors}$ = \Sexpr{mod.fin$coefficients["Minors"]}: If children under the age of 18 are living in the household of the customer, the odds to buy the magazine are increased multiplicatively by the factor $\exp(\beta_{Minors})=\Sexpr{odds_min}$\\

\textbf{Own} $\beta_{Own}$ = \Sexpr{mod.fin$coefficients["Own"]}: If the customer owns his residence, the odds to buy the magazine are increased multiplicatively by the factor $\exp(\beta_{Own})=\Sexpr{odds_own}$\\

\textbf{House} $\beta_{House}$ = \Sexpr{mod.fin$coefficients["House"]}: If the customer reside in a single family house, the odds to buy the magazine are decreased multiplicatively by the factor $\exp(\beta_{House})= \Sexpr{odds_house}$\\

\textbf{White} $\beta_{White}$ = \Sexpr{mod.fin$coefficients["White"]}: If the customer’s race is white, the odds to buy the magazine are increased multiplicatively by the factor $\exp(\beta_{White})=\Sexpr{odds_whi}$\\

\textbf{English} $\beta_{English}$ = \Sexpr{mod.fin$coefficients["English"]}: If the customer’s native language is english, the odds to buy the magazine are increased multiplicatively by the factor $\exp(\beta_{English})= \Sexpr{odds_eng}$\\

\textbf{Prev.Art.Mag} $\beta_{Prev.Art.Mag}$ = \Sexpr{mod.fin$coefficients["Prev.Art.Mag"]}: If the customer has previously bought an art magazine, the odds to buy the magazine are increased multiplicatively by the factor $\exp(\beta_{Prev.Art.Mag}) =\Sexpr{odds_prart}$\\
