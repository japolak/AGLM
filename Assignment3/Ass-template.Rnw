...
=======
>>>>>>> afa084beca2d566749033e366d248cf06a6850a3
...
We see a case of overdispersion in our data. We can test it following test:

<<t3-overdisp>>=
dispersiontest(mod2,alternative="greater",trafo=2)
@

We reject the null hypotheses that our data are equidispersed since p-value is <0.05, therefore, true dispersion parameter $\alpha$ is non zero. We can fit a negative binomial regression model which has a dispersion parameter $\theta$ (different from $\alpha$) to better fit the variance of data.
