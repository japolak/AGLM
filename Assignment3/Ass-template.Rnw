...
P-value of \texttt{<2e-16} shows that we can reject null hypothesis that means of types are the same.
...
<<t3-predicted>>=
predict(mod1, newdata = data.frame(type=factor(c(1,2,3), levels=c("1", "2", "3"), labels=c("elective", "urgent", "emergency"))), type="response")
@

Where columns corresponds to los for elective stay, urgent stay and emergency stay respectively. We can see that length of emergency stay is the longest and elective the shortests as expected. Difference between elective and urgent is 2.37 days and difference between urgent and emergency is 7.04. Even though difference in factors is low, since expected value of poisson regression is exponential function, differences in lenghts of stay are large.
...
From diagnostic plots we can find out we have two levarage points.
...
The dispersion parameter $\theta$ is equal to . Variance of the data is $\lambda + \frac{\lambda^2}{\theta}$. Finally we compare two tests:

<<t3-nbm_comp>>=
anova(mod2, mod3, test="Chisq")
@

We see that deviance of 3rd model is lower than that of mod2, for the same number of parameters. We therefore prioritize 3rd model over second one.
