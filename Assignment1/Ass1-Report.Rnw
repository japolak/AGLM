...
>>>>>>> 68c7a4273e5b0b4141e7e3002d0c920b8408b356
...
3) Here, we estimate the model described below and interpret the coefficients following way
...
\begin{itemize}
\item $\beta_0$ (Intercept): Expected net rent in Eur for an apartment with area being 0 in average location “0”.\\
\item $\beta_ 1$ (Area): A change in expected net rent in Eur for a 1 unit change in area given an average location.\\
\item $\beta_2$ (Location1): The difference in expected net rent for two equivalent apartments when one is in average location "0" and one is in good location "1". As $\beta_2 > 0$ expected net rent increases if location is changed from "0" to "1" (average to top), all else equal.\\
\item $\beta_3$ (Location2): The difference in expected net rent for two equivalent apartments when one is in location "0" and one is in location "2". As $\beta_3 > 0$ expected net rent increases if location is changed from "0" to "2" (average to top), all else equal.\\
